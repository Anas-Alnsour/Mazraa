...
<head>
    ...
    <title>Transport Company Portal - {{ config('app.name', 'Mazraa') }}</title>
...
<body class="bg-white antialiased text-gray-900 relative overflow-hidden min-h-screen" x-data="{ mounted: false }" x-init="setTimeout(() => mounted = true, 100)">
    ...
    <div class="flex flex-col lg:flex-row w-full min-h-screen">
        ...
        <div class="hidden lg:flex lg:w-[55%] relative overflow-hidden bg-[#020617] h-screen sticky top-0">
            ...
            <img src="https://images.unsplash.com/photo-1601584115197-04ecc0da31d7?q=80&w=2000&auto=format&fit=crop"
                 alt="Transport Background"
                 class="absolute inset-0 w-full h-full object-cover opacity-60 animate-slow-zoom">

            {{-- Deep Gradient Overlay (Dark Gray/Rose Tint) --}}
            ...
            {{-- Glowing Orbs (Transport Colors: Rose/Red) --}}
            <div class="absolute top-1/4 left-1/4 w-96 h-96 bg-rose-500/20 rounded-full blur-[120px] animate-float pointer-events-none z-0"></div>
            <div class="absolute bottom-1/4 right-1/4 w-96 h-96 bg-red-600/20 rounded-full blur-[120px] animate-float pointer-events-none z-0" style="animation-delay: 2s;"></div>
            ...
            <div class="relative z-20 flex flex-col justify-end p-16 w-full h-full pb-24">
                <div class="glass-panel p-10 rounded-[2.5rem] max-w-xl transition-all duration-700 transform"
                    ...
                    <div class="mb-8 flex items-center gap-4">
                        ...
                        <h2 class="text-3xl font-black text-white tracking-tighter">Mazraa<span class="text-rose-400">.com</span></h2>
                    </div>
                    ...
                    <p class="text-lg text-gray-200 font-medium leading-relaxed drop-shadow-md">
                        Manage your fleet, track drivers, and coordinate logistics efficiently within the Mazraa ecosystem.
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="w-full lg:w-[45%] flex flex-col relative bg-white overflow-y-auto min-h-screen">
            ...
            <div class="absolute top-8 left-8 sm:left-12 z-20">
                <a href="{{ url('/') }}" class="group flex items-center gap-2 text-xs font-black uppercase tracking-widest text-gray-400 hover:text-rose-600 transition-colors duration-300">
            ...
            <div class="flex-1 flex flex-col justify-center px-8 sm:px-16 lg:px-20 xl:px-28 py-24 min-h-full">
                ...
                <div class="lg:hidden mb-12 flex items-center gap-3 transition-all duration-700 transform" :class="mounted ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'">
                    <div class="bg-gradient-to-r from-rose-500 to-red-600 p-2.5 rounded-xl text-white shadow-lg">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M13 10V3L4 14h7v7l9-11h-7z"></path></svg>
                    </div>
                    <span class="text-3xl font-black tracking-tighter text-gray-900">Mazraa<span class="text-rose-500">.com</span></span>
                </div>
                ...
                <div class="mb-10 transition-all duration-700 delay-100 transform" :class="mounted ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'">
                    ...
                    <p class="text-base text-gray-500 font-medium">
                        Access your fleet and logistics management dashboard.
                    </p>
                </div>
                ...
                <form method="POST" action="{{ route('login') }}" class="space-y-6">
                    ...
                    <div class="group transition-all duration-700 delay-200 transform" :class="mounted ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'">
                        <label for="email" class="block text-[11px] font-black uppercase tracking-widest text-gray-500 mb-2 transition-colors group-focus-within:text-rose-600">Email Address</label>
                        <div class="relative">
                    ...
                    <div class="group transition-all duration-700 delay-300 transform" :class="mounted ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'">
                        <div class="flex items-center justify-between mb-2">
                            <label for="password" class="block text-[11px] font-black uppercase tracking-widest text-gray-500 transition-colors group-focus-within:text-rose-600">Password</label>
                            @if (Route::has('password.request'))
                                <a href="{{ route('password.request') }}" class="text-[11px] font-black uppercase tracking-widest text-rose-600 hover:text-rose-700 transition-colors">
                                    Forgot password?
                                </a>
                            @endif
                        </div>
                        <div class="relative">
                            ...
                            <input id="password" name="password" type="password" autocomplete="current-password" required
                                class="pl-14 block w-full bg-gray-50 border border-gray-200 text-gray-900 rounded-2xl shadow-sm focus:bg-white focus:ring-4 focus:ring-rose-500/20 focus:border-rose-500 sm:text-base py-4 font-medium transition-all duration-300 {{ $errors->has('password') ? 'border-red-500 ring-4 ring-red-500/20' : '' }}"
                                placeholder="••••••••">
                        </div>
                    ...
                    <div class="flex items-center pt-2 transition-all duration-700 delay-400 transform" :class="mounted ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'">
                        <label class="relative inline-flex items-center cursor-pointer group/toggle">
                            ...
                            <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-rose-500/30 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-rose-500"></div>
