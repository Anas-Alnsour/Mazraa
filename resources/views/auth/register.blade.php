...
@php
    ...
    $focusRing = $isPartner ? 'focus:ring-blue-600/20 focus:border-blue-600' : 'focus:ring-[#1d5c42]/20 focus:border-[#1d5c42]';
    $btnGradient = $isPartner ? 'from-blue-600 to-indigo-700 hover:from-indigo-700 hover:to-blue-900 shadow-[0_10px_25px_rgba(37,99,235,0.4)]' : 'from-[#1d5c42] to-[#154230] hover:from-[#154230] hover:to-[#0a1c14] shadow-[0_10px_25px_rgba(29,92,66,0.4)]';
    ...
    $bgImage = $isPartner ? 'https://images.unsplash.com/photo-1497366216548-37526070297c?q=80&w=2000&auto=format&fit=crop' : asset('backgrounds/home.JPG');
    ...
    $portalSubtitle = $isPartner ? 'Enter your details to create a partner dashboard.' : 'Enter your details to access premium farm escapes.';
    $heroTitle = $isPartner ? 'Monetize Your' : 'Grow with';
    $heroSubtitle = $isPartner ? 'Farm Property.' : 'Our Community.';
    $heroDesc = $isPartner ? 'Join hundreds of top-tier farms securely managing bookings, dynamic pricing, and direct payouts in one dashboard.' : 'Create an account today to seamlessly book premium farm retreats, order essential supplies, and manage your logistics.';
...
<body class="bg-white antialiased text-gray-900 relative overflow-hidden h-screen" x-data="{ mounted: false }" x-init="setTimeout(() => mounted = true, 100)">
    ...
    <div class="flex w-full h-full">
        ...
        <div class="hidden lg:flex lg:w-[45%] xl:w-[50%] relative overflow-hidden bg-[#020617]">
            <img src="{{ $bgImage }}" alt="Background" class="absolute inset-0 w-full h-full object-cover opacity-[0.45] animate-slow-zoom {{ !$isPartner ? 'grayscale-[20%]' : '' }}">
            <div class="absolute inset-0 bg-gradient-to-tr {{ $bgGradientFrom }} {{ $bgGradientVia }} to-transparent"></div>
            <div class="absolute top-1/4 left-1/4 w-96 h-96 {{ $blob1 }} rounded-full blur-[120px] animate-float pointer-events-none"></div>
            <div class="absolute bottom-1/4 right-1/4 w-96 h-96 {{ $blob2 }} rounded-full blur-[120px] animate-float pointer-events-none" style="animation-delay: 2s;"></div>

            <div class="absolute top-12 left-12 glass-panel rounded-full px-6 py-2.5 flex items-center gap-3 animate-float" style="animation-duration: 4s;">
                ...
                <div class="glass-panel p-10 rounded-[2.5rem] max-w-xl transition-all duration-700 transform" :class="mounted ? 'translate-y-0 opacity-100' : 'translate-y-12 opacity-0'">
                    <div class="mb-8 flex items-center gap-4">
                        <div class="p-3.5 bg-white/10 rounded-2xl border border-white/20 shadow-inner backdrop-blur-md">
                            ...
                            @else
                                <svg class="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"></path></svg>
                            @endif
                        </div>
                        <h2 class="text-3xl font-black text-white tracking-tighter">Mazraa<span class="{{ $iconColor }}">.com</span></h2>
                    </div>
        ...
        {{-- ==========================================
             ...
             ========================================== --}}
        <div class="w-full lg:w-[55%] xl:w-[50%] flex flex-col relative bg-white h-full overflow-y-auto custom-scrollbar">

            {{-- Back to Home Button --}}
            <div class="absolute top-8 left-8 sm:left-12 z-20 bg-white/90 backdrop-blur-sm p-1 rounded-full shadow-sm">
                <a href="{{ url('/') }}" class="group flex items-center gap-2 text-xs font-black uppercase tracking-widest text-gray-400 {{ $textAccentHover }} transition-colors duration-300">
            ...
            <div class="flex-1 flex flex-col justify-center px-8 sm:px-16 lg:px-20 xl:px-28 py-20 min-h-full">
                ...
                <div class="mb-10 transition-all duration-700 delay-100 transform" :class="mounted ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'">
                    ...
                    <p class="text-base text-gray-500 font-medium">
                        ...
                        <a href="{{ $loginUrl }}" class="font-black {{ $textAccent }} {{ $textAccentHover }} transition-colors ml-1 border-b-2 border-transparent {{ $borderAccentHover }} pb-0.5">
                            Sign in instead
                        </a>
                    </p>
                </div>
                ...
                <form method="POST" action="{{ $postUrl }}" class="space-y-5">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 transition-all duration-700 delay-200 transform" :class="mounted ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'">
                        {{-- Full Name Input --}}
                        <div class="group">
                            <label for="name" class="block text-[11px] font-black uppercase tracking-widest text-gray-500 mb-2 transition-colors group-focus-within:{{ $textAccent }}">Full Name</label>
                            <div class="relative">
                                <div class="absolute inset-y-0 left-0 pl-5 flex items-center pointer-events-none">
                                    <svg class="h-5 w-5 text-gray-400 group-focus-within:{{ $textAccent }} transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        {{-- Phone Input --}}
                        <div class="group">
                            <label for="phone" class="block text-[11px] font-black uppercase tracking-widest text-gray-500 mb-2 transition-colors group-focus-within:{{ $textAccent }}">Phone Number</label>
                            <div class="relative">
                                <div class="absolute inset-y-0 left-0 pl-5 flex items-center pointer-events-none">
                                    <svg class="h-5 w-5 text-gray-400 group-focus-within:{{ $textAccent }} transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="group transition-all duration-700 delay-300 transform" :class="mounted ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'">
                        <label for="email" class="block text-[11px] font-black uppercase tracking-widest text-gray-500 mb-2 transition-colors group-focus-within:{{ $textAccent }}">Email Address</label>
                        <div class="relative">
                            <div class="absolute inset-y-0 left-0 pl-5 flex items-center pointer-events-none">
                                <svg class="h-5 w-5 text-gray-400 group-focus-within:{{ $textAccent }} transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    {{-- Passwords Grid --}}
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 transition-all duration-700 delay-[400ms] transform" :class="mounted ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'">
                        <div class="group">
                            <label for="password" class="block text-[11px] font-black uppercase tracking-widest text-gray-500 mb-2 transition-colors group-focus-within:{{ $textAccent }}">Password</label>
                            <div class="relative">
                                <div class="absolute inset-y-0 left-0 pl-5 flex items-center pointer-events-none">
                                    <svg class="h-5 w-5 text-gray-400 group-focus-within:{{ $textAccent }} transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <input id="password" name="password" type="password" autocomplete="new-password" required
                                    class="pl-14 block w-full bg-gray-50 border border-gray-200 text-gray-900 rounded-2xl shadow-sm focus:bg-white {{ $focusRing }} sm:text-base py-4 font-medium transition-all duration-300 {{ $errors->has('password') ? 'border-red-500 ring-4 ring-red-500/20' : '' }}"
                                    placeholder="••••••••">
                            </div>
                        ...
                        <div class="group">
                            <label for="password_confirmation" class="block text-[11px] font-black uppercase tracking-widest text-gray-500 mb-2 transition-colors group-focus-within:{{ $textAccent }}">Confirm</label>
                            <div class="relative">
                                <div class="absolute inset-y-0 left-0 pl-5 flex items-center pointer-events-none">
                                    <svg class="h-5 w-5 text-gray-400 group-focus-within:{{ $textAccent }} transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <input id="password_confirmation" name="password_confirmation" type="password" required autocomplete="new-password"
                                    class="pl-14 block w-full bg-gray-50 border border-gray-200 text-gray-900 rounded-2xl shadow-sm focus:bg-white {{ $focusRing }} sm:text-base py-4 font-medium transition-all duration-300"
                                    placeholder="••••••••">
                            </div>
                    ...
                    {{-- ==========================================
                         ...
                         ========================================== --}}
                    @if($isPartner)
                    <div class="pt-4 transition-all duration-700 delay-500 transform" :class="mounted ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'">
                        <div class="bg-gray-50 rounded-3xl p-6 sm:p-8 border border-gray-200 shadow-inner relative overflow-hidden">
                            ...
                            <div class="flex items-center gap-3 mb-6 relative z-10">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-gray-500 mt-0.5">Where should we send your earnings? <span class="italic font-semibold text-blue-600">(Optional for now)</span></p>
                                </div>
                            </div>
                            ...
                            <div class="space-y-5 relative z-10">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                    <div class="group">
                                        <label for="bank_name" class="block text-[10px] font-black uppercase tracking-widest text-gray-500 mb-2 transition-colors group-focus-within:text-blue-600">Bank Name</label>
                                    ...
                                    <div class="group">
                                        <label for="account_holder_name" class="block text-[10px] font-black uppercase tracking-widest text-gray-500 mb-2 transition-colors group-focus-within:text-blue-600">Account Holder</label>
                                ...
                                <div class="group">
                                    <label for="iban" class="block text-[10px] font-black uppercase tracking-widest text-gray-500 mb-2 transition-colors group-focus-within:text-blue-600">IBAN Number</label>
                                    <div class="relative">
                    ...
                    {{-- Terms Checkbox --}}
                    <div class="flex items-center pt-2 transition-all duration-700 delay-[600ms] transform" :class="mounted ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'">
                    ...
                    {{-- Submit Button --}}
                    <div class="pt-6 transition-all duration-700 delay-[700ms] transform" :class="mounted ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'">
                        <button type="submit"
