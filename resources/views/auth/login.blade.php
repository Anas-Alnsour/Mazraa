...
@php
    ...
    $focusRing = $isPortal ? 'focus:ring-blue-600/20 focus:border-blue-600' : 'focus:ring-[#1d5c42]/20 focus:border-[#1d5c42]';
    $btnGradient = $isPortal ? 'from-blue-600 to-indigo-700 hover:from-indigo-700 hover:to-blue-900 shadow-[0_10px_25px_rgba(37,99,235,0.4)]' : 'from-[#1d5c42] to-[#154230] hover:from-[#154230] hover:to-[#0a1c14] shadow-[0_10px_25px_rgba(29,92,66,0.4)]';
    ...
    $bgImage = $isPortal ? 'https://images.unsplash.com/photo-1497366216548-37526070297c?q=80&w=2000&auto=format&fit=crop' : asset('backgrounds/login&register.JPG');
@endphp
    ...
    <div class="flex w-full h-full">
        ...
        <div class="hidden lg:flex lg:w-[55%] relative overflow-hidden bg-[#020617]">
           ...
           {{-- Animated Background Image (Adjusted for better visibility) --}}
            <img src="{{ $bgImage }}" alt="Background" class="absolute inset-0 w-full h-full object-cover opacity-50 animate-slow-zoom">
        ...
        <div class="w-full lg:w-[45%] flex flex-col relative bg-white h-full overflow-y-auto">
            ...
            <div class="absolute top-8 left-8 sm:left-12 z-20">
                <a href="{{ url('/') }}" class="group flex items-center gap-2 text-xs font-black uppercase tracking-widest text-gray-400 hover:{{ $textAccent }} transition-colors duration-300">
            ...
            <div class="flex-1 flex flex-col justify-center px-8 sm:px-16 lg:px-20 xl:px-28 py-20 min-h-full">
                ...
                <form method="POST" action="{{ $isPortal ? route('portal.login') : route('login') }}" class="space-y-6">
                    ...
                    <div class="group transition-all duration-700 delay-300 transform" :class="mounted ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'">
                        <div class="flex items-center justify-between mb-2">
                            <label for="password" class="block text-[11px] font-black uppercase tracking-widest text-gray-500 transition-colors group-focus-within:{{ $textAccent }}">Password</label>
                            @if (Route::has('password.request'))
                                <a href="{{ route('password.request') }}" class="text-[11px] font-black uppercase tracking-widest {{ $textAccent }} {{ $textAccentHover }} transition-colors">
                                    Forgot password?
                                </a>
                            @endif
                        </div>
                ...
                {{-- Footer Links --}}
                <div class="mt-12 text-center transition-all duration-700 delay-[600ms] transform" :class="mounted ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'">
                    @if(!$isPortal)
                        <p class="text-sm font-medium text-gray-500">
                            New to Mazraa?
                            <a href="{{ route('register') }}" class="font-black {{ $textAccent }} {{ $textAccentHover }} transition-colors ml-1 border-b-2 border-transparent {{ $borderAccentHover }} pb-0.5">
                                Create an account
                            </a>
                        </p>
                    @else
                        <p class="text-sm font-medium text-gray-500">
                            ...
                            <a href="{{ route('partner.register') }}" class="font-black {{ $textAccent }} {{ $textAccentHover }} transition-colors ml-1 border-b-2 border-transparent {{ $borderAccentHover }} pb-0.5">
                                Partner with us
                            </a>
                        </p>
                    @endif
