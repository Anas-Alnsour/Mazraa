...
@section('title', 'Deploy New Product')
...
<div class="max-w-[96%] xl:max-w-5xl mx-auto space-y-10 pb-24 animate-god-in fade-in-up">
    ...
    <div class="relative overflow-hidden flex flex-col md:flex-row md:items-center justify-between gap-6 bg-slate-900/80 p-8 md:p-10 rounded-[3rem] border border-slate-800 backdrop-blur-2xl shadow-2xl">
        ...
        <div class="relative z-10">
            <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-slate-950/50 border border-slate-700 text-[9px] font-black uppercase tracking-widest mb-3 shadow-inner text-slate-400">
                <span class="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span> Inventory Expansion
            </div>
            <h1 class="text-4xl md:text-5xl font-black text-white tracking-tighter mb-2 leading-none">Deploy New <span class="bg-clip-text text-transparent bg-gradient-to-r from-teal-400 to-emerald-400">Product</span></h1>
            <p class="text-slate-400 font-bold uppercase tracking-widest text-[10px] mt-3">Add a new item to your supply catalog.</p>
        </div>
    ...
    {{-- 🌟 Main Form --}}
    <form action="{{ route('supplies.items.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8 relative z-10">
        @csrf
        ...
        <div class="bg-slate-900/60 p-8 md:p-12 rounded-[3rem] border border-slate-800 backdrop-blur-xl shadow-2xl relative overflow-hidden">
            ...
            <div class="space-y-3">
                <label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest ml-2">Main Display Image <span class="text-emerald-500">*</span></label>
                <div class="relative group h-64 rounded-[2rem] border-2 border-dashed border-slate-700 bg-[#020617] hover:border-emerald-500 hover:bg-emerald-500/5 transition-all shadow-inner cursor-pointer flex flex-col items-center justify-center" onclick="document.getElementById('image').click()">
                    <input id="image" name="image" type="file" class="hidden" accept="image/*">
                    <div class="text-center transform transition-all duration-500 group-hover:scale-110">
                        <div class="w-16 h-16 rounded-full bg-slate-800 flex items-center justify-center mx-auto mb-4 text-slate-500 group-hover:text-emerald-400 group-hover:bg-emerald-500/20 border border-slate-700 group-hover:border-emerald-500/30 transition-colors shadow-lg">
                            <svg class="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/></svg>
                        </div>
                        <span class="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] group-hover:text-emerald-400 transition-colors">Upload Product File</span>
                        <p class="text-[9px] font-bold text-slate-600 mt-2 uppercase tracking-widest">PNG, JPG, WEBP up to 5MB</p>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="md:col-span-2 space-y-2">
                    ...
                    <div class="relative group">
                        ...
                        <select name="category" id="category" required class="w-full dark-select pl-14 py-4 font-bold text-sm">
                            ...
                            <option value="Meat & BBQ" {{ old('category') == 'Meat & BBQ' ? 'selected' : '' }}>Meat & BBQ</option>
                            <option value="Charcoal & Firewood" {{ old('category') == 'Charcoal & Firewood' ? 'selected' : '' }}>Charcoal & Firewood</option>
                            <option value="Beverages & Snacks" {{ old('category') == 'Beverages & Snacks' ? 'selected' : '' }}>Beverages & Snacks</option>
                ...
                <div class="md:col-span-2 space-y-2">
                    ...
                    <div class="relative group">
                        ...
                        <textarea name="description" id="description" rows="5" required
                            placeholder="Detail the product specifications, weight, and contents..."
                            class="w-full supply-input pl-14 py-5 resize-none leading-relaxed font-medium">{{ old('description') }}</textarea>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="p-8 bg-[#020617] rounded-[2rem] border border-emerald-500/30 shadow-[inset_0_2px_15px_rgba(0,0,0,0.5)] group hover:border-emerald-500/50 transition-all relative overflow-hidden">
                    ...
                    <div class="relative">
                        ...
                        <input type="number" step="0.01" name="price" id="price" value="{{ old('price') }}" min="0" required
                            class="w-full bg-transparent border-none text-white rounded-xl focus:ring-0 pl-16 py-2 text-4xl font-black outline-none" placeholder="0.00">
                    </div>
                </div>
                ...
                <div class="space-y-2 flex flex-col h-full justify-end">
                    ...
                    <div class="relative group h-[calc(100%-24px)] min-h-[4rem]">
                        ...
                        <input type="number" name="stock" id="stock" value="{{ old('stock', 0) }}" min="0" required
                            class="w-full supply-input h-full pl-16 text-3xl font-black" placeholder="0">
                    </div>
                </div>
            </div>
        </div>
