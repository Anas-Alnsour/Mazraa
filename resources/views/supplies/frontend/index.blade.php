        ...
        <div class="max-w-[96%] xl:max-w-7xl mx-auto relative z-30 -mt-20">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-center bg-white p-4 rounded-[2rem] shadow-[0_10px_40px_rgba(0,0,0,0.04)] border border-gray-100 mb-12 gap-4 fade-in-up" style="animation-delay: 0.2s;">
                ...
                <div class="flex gap-3 w-full sm:w-auto">
                    <a href="{{ route('orders.my_orders') }}" class="flex-1 sm:flex-none justify-center px-6 py-4 bg-gray-50 border border-gray-200 text-gray-600 font-black text-[10px] md:text-xs uppercase tracking-widest rounded-xl hover:bg-white hover:text-[#1d5c42] hover:border-[#1d5c42]/30 transition-colors shadow-sm flex items-center gap-2 active:scale-95">
                        <svg class="w-4 h-4 text-[#c2a265]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"></path></svg>
                        Track Orders
                    </a>
                    <a href="{{ route('cart.view') }}" class="flex-1 sm:flex-none justify-center px-6 py-4 bg-gradient-to-tr from-[#1d5c42] to-[#14402e] hover:to-[#0f3022] text-white font-black text-[10px] md:text-xs uppercase tracking-widest rounded-xl shadow-lg shadow-[#1d5c42]/30 hover:shadow-xl hover:shadow-[#1d5c42]/40 transition-all flex items-center gap-2 active:scale-95 border border-white/10">
            ...
            @if(session('success'))
                <div class="mb-8 bg-emerald-50 border border-emerald-200 p-5 rounded-2xl flex items-center justify-between shadow-sm fade-in-up">
                    ...
                    <a href="{{ route('cart.view') }}" class="hidden sm:block px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white text-[10px] font-black uppercase tracking-widest rounded-xl transition-colors shadow-lg">View Cart</a>
                </div>
            @endif

            @if(session('error'))
                <div class="mb-8 bg-rose-50 border border-rose-200 p-5 rounded-2xl flex items-center gap-4 shadow-sm fade-in-up">
            ...
            @auth
                @if(auth()->user()->role === 'user' && empty($eligibleBookings))
                    <div class="mb-10 bg-amber-50/80 border border-amber-200 rounded-[2rem] p-6 shadow-sm flex items-start gap-4 fade-in-up">
                        ...
                        <div>
                            ...
                            <a href="{{ route('explore') }}" class="inline-block mt-4 px-6 py-3 bg-amber-500 hover:bg-amber-600 text-white text-[10px] font-black uppercase tracking-widest rounded-xl transition-colors shadow-lg shadow-amber-500/20 active:scale-95">Book a Farm First</a>
                        </div>
                    </div>
                @endif
            @endauth
            ...
            @else
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 lg:gap-8">
                    @foreach ($supplies as $index => $supply)
                        <div class="group bg-white rounded-[2.5rem] shadow-[0_10px_40px_rgba(0,0,0,0.03)] hover:shadow-[0_25px_50px_rgba(29,92,66,0.1)] border border-gray-100 hover:border-[#1d5c42]/20 overflow-hidden transition-all duration-500 flex flex-col h-full fade-in-up-stagger" style="animation-delay: {{ $index * 0.05 }}s">

                            {{-- 📷 Image Section (Framed) --}}
                            <div class="p-3 pb-0">
                                <div class="relative h-56 overflow-hidden rounded-[1.5rem] bg-gray-100 shadow-inner">
                                    ...
                                    {{-- Category Badge --}}
                                    @if($supply->category)
                                        <div class="absolute top-3 left-3 bg-black/40 backdrop-blur-md text-white text-[9px] font-black px-3 py-1.5 rounded-xl uppercase tracking-widest border border-white/20 shadow-lg">
                                            {{ $supply->category }}
                                        </div>
                                    @endif
                                </div>
                            ...
                            {{-- 📝 Content Section --}}
                            <div class="px-5 md:px-6 pt-5 pb-6 flex flex-col flex-1">
                                <h3 class="text-lg font-black text-gray-900 group-hover:text-[#1d5c42] transition-colors leading-tight line-clamp-1 truncate tracking-tight mb-2">
                                    {{ $supply->name }}
                                </h3>
                                ...
                                <div class="mb-5 flex items-baseline gap-1 bg-gray-50/50 w-max px-3 py-1.5 rounded-xl border border-gray-100">
                                    <span class="text-xl font-black text-[#1d5c42] tracking-tighter">{{ number_format($supply->price, 2) }}</span>
                                    <span class="text-[9px] font-black text-gray-400 uppercase tracking-widest">JOD</span>
                                </div>
                                ...
                                <div class="mt-auto border-t border-gray-100 pt-4">
                                    @auth
                                        @if(auth()->user()->role === 'user')
                                            <form action="{{ route('cart.add', $supply->id) }}" method="POST" class="flex items-center gap-2">
                                                @csrf
                                                {{-- Sleek Qty Selector --}}
                                                <div class="flex items-center bg-gray-50 border border-gray-200 rounded-xl p-1 shadow-inner h-11" x-data="{ qty: 1 }">
                                                    <button type="button" @click="if(qty > 1) qty--" class="w-8 h-full flex items-center justify-center rounded-lg bg-white text-gray-600 shadow-sm hover:text-rose-500 transition-all font-black text-sm active:scale-95 focus:outline-none">-</button>
                                                    <input type="number" name="quantity" x-model="qty" readonly class="w-8 text-center bg-transparent border-none focus:ring-0 text-xs font-black text-gray-900 p-0 pointer-events-none">
                                                    <button type="button" @click="if(qty < {{ $supply->stock }}) qty++" class="w-8 h-full flex items-center justify-center rounded-lg bg-white text-gray-600 shadow-sm hover:text-[#1d5c42] transition-all font-black text-sm active:scale-95 focus:outline-none">+</button>
                                                </div>

                                                {{-- Add Button --}}
                                                <button type="submit" class="flex-1 h-11 bg-[#1d5c42] hover:bg-[#14402e] text-white font-black rounded-xl transition-all duration-300 shadow-[0_8px_15px_rgba(29,92,66,0.2)] hover:shadow-[0_12px_20px_rgba(29,92,66,0.3)] active:scale-95 flex items-center justify-center gap-1.5 group/btn focus:outline-none">
                                                    <svg class="w-3.5 h-3.5 transition-transform group-hover/btn:scale-110" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"></path></svg>
                                                    <span class="text-[9px] md:text-[10px] uppercase tracking-widest">Add</span>
                                                </button>
                                            </form>
                                        @else
                                            <div class="w-full h-11 bg-gray-50 border border-gray-100 text-gray-400 font-black text-[10px] flex items-center justify-center rounded-xl uppercase tracking-widest shadow-inner">
                                                Customer Only
                                            </div>
                                        @endif
                                    @else
                                        <a href="{{ route('login') }}" class="block w-full h-11 flex items-center justify-center text-center bg-gray-900 hover:bg-black text-white font-black rounded-xl transition-all shadow-lg uppercase tracking-widest text-[10px] active:scale-95">
                                            Login to Add
                                        </a>
                                    @endauth
                                </div>
        ...
        @auth
            ...
            @if(session('success') || $hasItemsInCart)
                <div class="fixed bottom-8 right-8 z-[90] fade-in-up delay-300">
                    <a href="{{ route('cart.view') }}" class="flex items-center gap-3 bg-gradient-to-r from-[#1d5c42] to-[#14402e] text-white px-6 py-4 rounded-2xl shadow-[0_20px_50px_rgba(29,92,66,0.5)] hover:shadow-[0_25px_60px_rgba(29,92,66,0.6)] transition-all duration-300 transform hover:-translate-y-1.5 group border border-white/10">
                        <div class="relative">
                            ...
                            <span class="absolute -top-2 -right-2 bg-yellow-500 text-gray-900 text-[9px] font-black w-4 h-4 rounded-full flex items-center justify-center shadow-sm animate-pulse">✓</span>
                        </div>
                        <span class="text-[10px] md:text-[11px] font-black uppercase tracking-widest text-white">Go to Cart</span>
                    </a>
                </div>
            @endif
        @endauth
