...
<div class="bg-[#f8fafc] min-h-screen pt-24 pb-12 font-sans selection:bg-[#1d5c42] selection:text-white">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 fade-in-up">
        ...
        <div class="text-center mb-10">
            <h1 class="text-3xl md:text-4xl font-black text-gray-900 tracking-tight mb-3">Complete Your Booking</h1>
            <p class="text-gray-500 font-medium">Choose your preferred payment method for <strong class="text-gray-900">{{ $farm->name }}</strong></p>
        </div>
        ...
        <div class="bg-white rounded-[2.5rem] p-8 md:p-10 shadow-[0_30px_60px_rgba(0,0,0,0.08)] border border-gray-100 relative overflow-hidden">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                {{-- Option 1: Credit Card / Visa --}}
                <form action="{{ route('payment.process.card', $booking->id) }}" method="POST" class="w-full">
                    @csrf
                ...
                <form action="{{ route('payment.process.cliq', $booking->id) }}" method="POST" class="w-full">
                    @csrf
            ...
            <div class="mt-8 text-center">
                <a href="{{ route('farms.show', $farm->id) }}" class="text-sm font-bold text-gray-400 hover:text-red-500 transition-colors">
                    Cancel and return to farm details
                </a>
            </div>
