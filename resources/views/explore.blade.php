...
@section('content')
    <div class="max-w-7xl mx-auto p-6">
        ...
        <form method="GET" action="{{ route('explore') }}" class="flex flex-wrap items-center gap-3 md:gap-4 mb-6">
            ...
            <select name="price_sort" class="h-10 w-36 md:w-40 border rounded px-3 pr-10">
            ...
            <select name="rating_sort" class="h-10 w-36 md:w-40 border rounded px-3 pr-10">
            ...
            <button type="submit" class="h-10 px-5 bg-green-600 text-white rounded hover:bg-green-700">
                Search
            </button>
        </form>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 items-stretch">
            ...
            @foreach ($farms as $farm)
                <div class="flex flex-col h-full bg-white rounded-2xl shadow-lg overflow-hidden transform transition duration-300 hover:scale-105 hover:shadow-2xl">

                    <div class="relative">
                        <img src="{{ $farm->main_image ? asset('storage/' . $farm->main_image) : 'https://via.placeholder.com/800x400' }}"
                            alt="{{ $farm->name }}" class="w-full h-52 sm:h-56 md:h-48 object-cover">
                        <div
                    ...
                    <div class="flex flex-col flex-1 p-5">
                        ...
                        <h2 class="text-2xl font-bold text-gray-900 mb-1 hover:text-green-600 transition">
                            {{ $farm->name }}</h2>
