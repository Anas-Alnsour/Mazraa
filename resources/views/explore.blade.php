...
<div class="max-w-7xl mx-auto p-6">
    ...
    <form method="GET" action="{{ route('explore') }}" class="flex flex-wrap gap-4 mb-6">
        <input type="text" name="name" placeholder="Search by name" value="{{ request('name') }}" class="border rounded px-3 py-2">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        @foreach($farms as $farm)
            <div class="bg-white shadow rounded overflow-hidden">
                <img src="{{ $farm->main_image ?? 'https://via.placeholder.com/400x200' }}" alt="{{ $farm->name }}" class="w-full h-48 object-cover">
                <div class="p-4">
                    ...
                    <div class="mt-4">
                        <a href="{{ route('farms.show', $farm->id) }}" class="block text-center bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700">View Details</a>
                    </div>
                </div>
            </div>
        @endforeach
