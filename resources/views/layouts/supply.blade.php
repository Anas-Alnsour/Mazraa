...
<head>
    ...
    <style>
        ...
        .sidebar-link:hover { transform: translateX(6px); border-left-color: #6366f1; background: rgba(30,41,59,0.5); }
        .sidebar-link-active {
            background: linear-gradient(to right, rgba(99,102,241,0.15), transparent);
            border-left-color: #6366f1;
            color: #818cf8 !important;
        }
        .sidebar-link-active svg { color: #818cf8 !important; }
    </style>
</head>
<body class="font-sans antialiased bg-[#020617] text-slate-300 selection:bg-indigo-500/30 selection:text-indigo-200 overflow-hidden" x-data="{ sidebarOpen: false }">

    {{-- Ambient Background Glows --}}
    <div class="fixed inset-0 z-0 pointer-events-none overflow-hidden">
        <div class="absolute -top-[20%] -left-[10%] w-[50vw] h-[50vw] bg-indigo-900/20 rounded-full blur-[120px] mix-blend-screen"></div>
        <div class="absolute bottom-[10%] -right-[10%] w-[40vw] h-[40vw] bg-blue-900/10 rounded-full blur-[150px] mix-blend-screen"></div>
    </div>
    ...
    <div class="flex h-screen relative z-10">
        ...
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
            ...
            <div class="flex items-center justify-center h-24 border-b border-slate-800/60 relative z-10 shrink-0">
                <a href="{{ route('supplies.dashboard') }}" class="flex items-center gap-4 group px-6 w-full">
                    <div class="w-11 h-11 rounded-2xl bg-gradient-to-br from-indigo-600 to-blue-800 flex items-center justify-center shadow-[0_0_20px_rgba(99,102,241,0.3)] group-hover:scale-105 group-hover:rotate-6 transition-all duration-500 border border-indigo-500/30">
                    ...
                    <div class="flex flex-col">
                        <span class="text-2xl font-black tracking-widest text-white uppercase leading-none group-hover:text-indigo-400 transition-colors">Mazraa</span>
                        <span class="text-[9px] font-black text-indigo-500 tracking-[0.4em] mt-1.5 uppercase">Supply Hub</span>
                    </div>
                </a>
            </div>
            ...
            <nav class="flex-1 px-4 py-8 space-y-2 overflow-y-auto custom-scrollbar relative z-10">
                ...
                <a href="{{ route('supplies.items.index') }}" class="sidebar-link flex items-center gap-4 px-4 py-3.5 rounded-xl font-bold text-[11px] uppercase tracking-widest text-slate-400 {{ request()->routeIs('supplies.items.*') ? 'sidebar-link-active' : '' }}">
            ...
            <header class="glass-nav sticky top-0 z-30 shrink-0">
                <div class="flex items-center justify-between px-6 lg:px-10 h-20">
                    ...
                    <div class="flex items-center gap-6">
                        <button @click="sidebarOpen = true" class="p-2.5 text-slate-400 rounded-xl lg:hidden bg-slate-900 border border-slate-800 transition-all hover:text-indigo-400 focus:outline-none">
                        ...
                        <div class="hidden lg:flex items-center gap-3">
                            <span class="text-[10px] font-black text-indigo-400 uppercase tracking-[0.3em] bg-indigo-500/10 border border-indigo-500/20 px-3 py-1.5 rounded-lg shadow-[0_0_15px_rgba(99,102,241,0.15)] flex items-center gap-2">
                    ...
                    <div class="hidden md:flex flex-col items-center justify-center absolute left-1/2 -translate-x-1/2 text-center w-full max-w-lg">
                        <span class="text-sm font-black text-white uppercase tracking-[0.4em] opacity-90 drop-shadow-md">@yield('title', 'Operations Core')</span>
                    </div>
                    ...
                    <div class="flex items-center gap-5 lg:gap-8">
                        ...
                        <div x-data="{ dropdownOpen: false }" class="relative">
                            <button @click="dropdownOpen = !dropdownOpen" @click.away="dropdownOpen = false"
                                    class="flex items-center gap-3 p-1.5 rounded-2xl transition-all duration-300 border border-slate-800 hover:border-indigo-500/50 bg-slate-900/50 group shadow-sm hover:shadow-md outline-none">
                                <div class="w-9 h-9 rounded-xl bg-gradient-to-tr from-indigo-600 to-blue-800 flex items-center justify-center text-white font-black text-sm shadow-[0_0_15px_rgba(99,102,241,0.4)] group-hover:scale-105 transition-transform">
                                    {{ substr(Auth::user()->name ?? 'V', 0, 1) }}
                                </div>
                                <div class="hidden lg:block text-left pr-4">
                                    <p class="text-xs font-black text-white uppercase tracking-widest leading-none">{{ Auth::user()->name ?? 'Vendor' }}</p>
                                    <p class="text-[9px] text-indigo-400 font-bold uppercase tracking-widest mt-1">Supply Partner</p>
                                </div>
                            </button>
                            ...
                            <div x-show="dropdownOpen"
                                ...
                                <div class="px-6 py-5 border-b border-slate-800/80 mb-2 bg-slate-800/20">
                                    <p class="text-[9px] font-black text-indigo-400 uppercase tracking-widest mb-1">Authenticated Entity</p>
                                    <p class="text-sm font-bold text-white truncate">{{ Auth::user()->email ?? 'vendor@mazraa.com' }}</p>
                                </div>
