    ...
    <aside :class="{'translate-x-0': sidebarOpen, '-translate-x-full': !sidebarOpen}" class="fixed inset-y-0 left-0 z-30 w-72 bg-slate-900 text-white transition-transform duration-300 lg:translate-x-0 lg:static lg:inset-0 flex flex-col shadow-2xl">
        <div class="flex items-center justify-center h-20 border-b border-slate-800">
            <a href="#" class="text-2xl font-black tracking-tighter text-white flex items-center">
                Mazraa<span class="text-blue-500">.Driver</span>
            </a>
        </div>
        ...
        <nav class="flex-1 px-4 py-8 space-y-3 overflow-y-auto">
            ...
            @if(Auth::user()->role === 'supply_driver')
                <p class="px-2 text-[10px] font-black text-slate-500 uppercase tracking-widest mb-4">Supply Delivery</p>
                <a href="{{ route('delivery.orders') }}" class="flex items-center gap-3 px-4 py-4 bg-blue-600 text-white rounded-2xl font-bold text-sm shadow-lg shadow-blue-900/20 transition-all transform active:scale-95">
            ...
            @if(Auth::user()->role === 'transport_driver')
                <p class="px-2 text-[10px] font-black text-slate-500 uppercase tracking-widest mb-4">Transport Trips</p>
                <a href="{{ route('shuttle.trips') }}" class="flex items-center gap-3 px-4 py-4 bg-blue-600 text-white rounded-2xl font-bold text-sm shadow-lg shadow-blue-900/20 transition-all transform active:scale-95">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8"></path></svg>
                    My Schedule
                </a>
            @endif

            <a href="#" class="flex items-center gap-3 px-4 py-4 text-slate-400 hover:bg-slate-800 hover:text-white rounded-2xl font-bold text-sm transition-all opacity-50 cursor-not-allowed mt-4">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                Trip History
            </a>

            <a href="#" class="flex items-center gap-3 px-4 py-4 text-slate-400 hover:bg-slate-800 hover:text-white rounded-2xl font-bold text-sm transition-all opacity-50 cursor-not-allowed">
        ...
        <div class="p-6 border-t border-slate-800">
            <div class="flex items-center gap-4 mb-6">
                <div class="w-12 h-12 rounded-full bg-slate-800 flex items-center justify-center text-blue-400 font-black text-xl border border-slate-700 shadow-inner">
                    {{ substr(Auth::user()->name, 0, 1) }}
                </div>
                <div>
                    <p class="text-sm font-bold text-white truncate max-w-[150px]">{{ Auth::user()->name }}</p>
                    <p class="text-[10px] text-emerald-400 uppercase tracking-widest mt-1 flex items-center gap-1.5">
        ...
        <header class="h-20 bg-white border-b border-gray-100 flex items-center justify-between px-6 lg:px-10 shadow-sm shrink-0">
            <div class="flex items-center gap-4">
                ...
                <div class="hidden sm:flex items-center gap-2">
                    <span class="text-[10px] font-black text-gray-400 uppercase tracking-widest bg-gray-100 px-3 py-1.5 rounded-lg">Fleet Operations</span>
                </div>
            </div>
