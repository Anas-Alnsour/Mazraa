            ...
            <div class="mt-6">
                ...
                <ul class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                    @php
                        ...
                        $booked_hours = $farm->bookings->pluck('start_time')->map(function ($dt) {
                            return \Carbon\Carbon::parse($dt)->format('H');
                        });
                    @endphp
                    @foreach ($all_hours as $hour)
                        <li
                            class="px-2 py-1 rounded text-center text-sm font-medium {{ $booked_hours->contains($hour) ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-700' }}">
