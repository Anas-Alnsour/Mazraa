...
<style>
    ...
    /* Radio Buttons */
    .status-radio:checked + label { border-color: #0891b2; background-color: rgba(8, 145, 178, 0.1); }
</style>
...
<div class="max-w-[96%] xl:max-w-4xl mx-auto space-y-10 pb-24 animate-god-in fade-in-up">
    ...
    <div class="relative overflow-hidden flex flex-col md:flex-row md:items-center justify-between gap-6 bg-slate-900/80 p-8 md:p-10 rounded-[3rem] border border-slate-800 backdrop-blur-2xl shadow-2xl">
        ...
        <div class="relative z-10">
            <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-slate-950/50 border border-slate-700 text-[9px] font-black uppercase tracking-widest mb-3 shadow-inner text-slate-400">
                <span class="w-1.5 h-1.5 rounded-full bg-blue-500 animate-pulse"></span> Asset Configuration
            </div>
            <h1 class="text-4xl md:text-5xl font-black text-white tracking-tighter mb-2 leading-none">Edit <span class="bg-clip-text text-transparent bg-gradient-to-r from-cyan-400 to-blue-400">Vehicle</span></h1>
            <p class="text-slate-400 font-bold uppercase tracking-widest text-[10px] mt-3">
                Updating plate: <span class="bg-slate-950 text-cyan-400 px-3 py-1.5 rounded-lg border border-slate-800 ml-1 font-mono tracking-widest">{{ $vehicle->license_plate }}</span>
            </p>
        </div>
    ...
    <div class="bg-slate-900/60 p-8 md:p-12 rounded-[3rem] border border-slate-800 backdrop-blur-xl shadow-2xl relative overflow-hidden">
        <form action="{{ route('transport.vehicles.update', $vehicle->id) }}" method="POST" class="space-y-10 relative z-10">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="md:col-span-2 space-y-2">
                    ...
                    <select name="type" id="type" required class="w-full dark-select px-5 py-4 font-bold text-sm">
                        <option value="Mini Van (7 Seats)" {{ old('type', $vehicle->type) == 'Mini Van (7 Seats)' ? 'selected' : '' }}>Mini Van (7 Seats)</option>
                        <option value="Van (12 Seats)" {{ old('type', $vehicle->type) == 'Van (12 Seats)' ? 'selected' : '' }}>Van (12 Seats)</option>
                        <option value="Coaster Bus (22 Seats)" {{ old('type', $vehicle->type) == 'Coaster Bus (22 Seats)' ? 'selected' : '' }}>Coaster Bus (22 Seats)</option>
                        <option value="Large Bus (50 Seats)" {{ old('type', $vehicle->type) == 'Large Bus (50 Seats)' ? 'selected' : '' }}>Large Bus (50 Seats)</option>
                        <option value="SUV / 4x4" {{ old('type', $vehicle->type) == 'SUV / 4x4' ? 'selected' : '' }}>SUV / 4x4</option>
                        <option value="Sedan (VIP)" {{ old('type', $vehicle->type) == 'Sedan (VIP)' ? 'selected' : '' }}>Sedan (VIP)</option>
                    </select>
                </div>
            ...
            <div class="pt-8 mt-8 border-t border-slate-800">
                <label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-4 ml-2">Current Status Matrix</label>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div class="relative">
                        <input type="radio" name="status" id="status_available" value="available" class="status-radio sr-only" {{ old('status', $vehicle->status) === 'available' ? 'checked' : '' }}>
                        <label for="status_available" class="flex flex-col items-center justify-center p-6 border border-slate-700 bg-slate-950 rounded-2xl cursor-pointer transition-all hover:border-emerald-500/50 group shadow-inner">
                    ...
                    <div class="relative">
                        <input type="radio" name="status" id="status_maintenance" value="maintenance" class="status-radio sr-only" {{ old('status', $vehicle->status) === 'maintenance' ? 'checked' : '' }}>
                        <label for="status_maintenance" class="flex flex-col items-center justify-center p-6 border border-slate-700 bg-slate-950 rounded-2xl cursor-pointer transition-all hover:border-amber-500/50 group shadow-inner">
