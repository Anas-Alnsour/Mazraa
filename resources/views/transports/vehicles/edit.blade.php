...
<div class="py-12 bg-slate-50 min-h-screen">
    <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            ...
            <h1 class="text-3xl font-black text-slate-900 tracking-tight">Edit Vehicle Details</h1>
            <p class="mt-2 text-sm font-bold text-slate-500 uppercase tracking-widest">Update information for plate: <span class="font-black text-slate-800 bg-slate-200 px-2 py-0.5 rounded">{{ $vehicle->license_plate }}</span></p>
        </div>
        ...
        <div class="bg-white rounded-[2.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 overflow-hidden">
            <form action="{{ route('transport.vehicles.update', $vehicle->id) }}" method="POST" class="p-8 sm:p-10">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div>
                        ...
                        <input type="text" name="license_plate" id="license_plate" value="{{ old('license_plate', $vehicle->license_plate) }}" required class="block w-full rounded-2xl border-slate-200 bg-slate-50 shadow-inner focus:border-cyan-500 focus:ring-4 focus:ring-cyan-500/10 sm:text-sm font-black text-slate-900 px-5 py-4 uppercase transition-all">
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="number" name="capacity" id="capacity" min="1" value="{{ old('capacity', $vehicle->capacity) }}" required class="block w-full rounded-2xl border-slate-200 bg-slate-50 shadow-inner focus:border-cyan-500 focus:ring-4 focus:ring-cyan-500/10 sm:text-sm font-bold text-slate-900 px-5 py-4 transition-all">
                ...
                <div class="pt-8 mt-8 border-t border-slate-100">
                    <label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-4 pl-1">Current Status</label>
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div class="relative">
                            <input type="radio" name="status" id="status_available" value="available" class="peer sr-only" {{ old('status', $vehicle->status) === 'available' ? 'checked' : '' }}>
                        ...
                        <div class="relative">
                            <input type="radio" name="status" id="status_maintenance" value="maintenance" class="peer sr-only" {{ old('status', $vehicle->status) === 'maintenance' ? 'checked' : '' }}>
                        ...
                        <div class="relative">
                            <input type="radio" name="status" id="status_booked" value="booked" class="peer sr-only" {{ old('status', $vehicle->status) === 'booked' ? 'checked' : '' }}>
