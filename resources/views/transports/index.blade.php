            ...
            <table class="min-w-full border border-gray-200 rounded-lg shadow">
                <thead class="bg-gray-100">
                    <tr>
                        ...
                        <th class="p-3 text-left">Start & Return Point</th>
                        <th class="p-3 text-left">Farm</th>
                        <th class="p-3 text-left">Distance (km)</th>
                        <th class="p-3 text-left">Price</th>
                        <th class="p-3 text-left">Farm Arrival Time</th>
                        <th class="p-3 text-left">Farm Departure Time</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach($transports as $transport)
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="p-3 border">{{ $transport->start_and_return_point }}</td>
                            <td class="p-3 border">{{ optional($transport->farm)->name ?? '-' }}</td>
                            <td class="p-3 border">{{ $transport->distance }}</td>
                            <td class="p-3 border font-semibold text-green-700">${{ $transport->price }}</td>
                            <td class="p-3 border">{{ $transport->Farm_Arrival_Time ?? '-' }}</td>
                            <td class="p-3 border">{{ $transport->Farm_Departure_Time ?? '-' }}</td>
                            <td class="p-3 border">
