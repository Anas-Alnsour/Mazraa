...
<div class="max-w-[96%] xl:max-w-4xl mx-auto space-y-10 pb-24 animate-god-in fade-in-up">
    ...
    <div class="relative overflow-hidden flex flex-col md:flex-row md:items-center justify-between gap-6 bg-slate-900/80 p-8 md:p-10 rounded-[3rem] border border-slate-800 backdrop-blur-2xl shadow-2xl">
        ...
        <div class="relative z-10">
            ...
            <h1 class="text-4xl md:text-5xl font-black text-white tracking-tighter mb-2 leading-none">Reconfigure <span class="bg-clip-text text-transparent bg-gradient-to-r from-cyan-400 to-blue-400">Operative</span></h1>
            <p class="text-slate-400 font-bold uppercase tracking-widest text-[10px] mt-3">
                Updating profile for: <span class="bg-slate-950 text-cyan-400 px-3 py-1.5 rounded-lg border border-slate-800 ml-1 font-black">{{ $driver->name }}</span>
            </p>
        </div>
    ...
    <div class="bg-slate-900/60 p-8 md:p-12 rounded-[3rem] border border-slate-800 backdrop-blur-xl shadow-2xl relative overflow-hidden">
        <form action="{{ route('transport.drivers.update', $driver->id) }}" method="POST" class="space-y-10 relative z-10">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="md:col-span-2 space-y-2">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $driver->name) }}" required
                        class="w-full transport-input px-5 py-4 font-bold text-sm">
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <input type="email" name="email" id="email" value="{{ old('email', $driver->email) }}" required
                        class="w-full transport-input px-5 py-4 font-bold font-mono text-sm">
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="phone" id="phone" value="{{ old('phone', $driver->phone) }}" required
                        class="w-full transport-input px-5 py-4 font-bold font-mono tracking-widest text-sm">
                </div>
                ...
                <div class="md:col-span-2 space-y-2">
                    ...
                    @else
                        <div class="px-5 py-4 bg-amber-500/10 text-amber-400 rounded-2xl text-sm font-bold border border-amber-500/20">
                            No vehicles available. <a href="{{ route('transport.vehicles.create') }}" class="underline text-amber-300">Add a vehicle</a> to this fleet first.
                        </div>
                    @endif
                </div>

                {{-- Security Protocol (Optional for Update) --}}
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 md:col-span-2 mt-8 p-6 md:p-8 bg-slate-950/50 rounded-[2rem] border border-slate-800 shadow-inner">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="password" name="password" id="password" placeholder="••••••••"
                            class="w-full transport-input px-5 py-4 font-bold tracking-widest focus:border-amber-500 focus:ring-amber-500">
                    </div>
