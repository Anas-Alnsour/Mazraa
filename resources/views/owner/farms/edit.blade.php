<x-dashboard-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-green-600 font-bold mt-1 uppercase tracking-widest">{{ $farm->name }}</p>
            </div>
            ...
            <form action="{{ route('owner.farms.update', $farm->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8" id="farmForm">
                ...
                <div class="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 p-8 sm:p-10">
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name', $farm->name) }}" required class="w-full rounded-2xl border-gray-200 bg-gray-50 focus:bg-white focus:ring-4 focus:ring-green-500/10 focus:border-green-500 py-4 px-6 transition-all font-bold text-gray-800">
                        </div>

                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-2 px-1">Description *</label>
                            <textarea name="description" rows="4" required class="w-full rounded-[2rem] border-gray-200 bg-gray-50 focus:bg-white focus:ring-4 focus:ring-green-500/10 focus:border-green-500 py-4 px-6 transition-all font-medium text-gray-800">{{ old('description', $farm->description) }}</textarea>
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label class="block text-sm font-bold text-gray-700 mb-2 px-1">Price Per Shift (JOD) *</label>
                                <input type="number" step="0.01" name="price_per_night" value="{{ old('price_per_night', $farm->price_per_night) }}" required class="w-full rounded-2xl border-gray-200 bg-gray-50 focus:bg-white focus:ring-4 focus:ring-green-500/10 focus:border-green-500 py-4 px-6 transition-all font-bold text-gray-800">
                            </div>
                            <div>
                                <label class="block text-sm font-bold text-gray-700 mb-2 px-1">Capacity (Guests) *</label>
                                <input type="number" name="capacity" value="{{ old('capacity', $farm->capacity) }}" required class="w-full rounded-2xl border-gray-200 bg-gray-50 focus:bg-white focus:ring-4 focus:ring-green-500/10 focus:border-green-500 py-4 px-6 transition-all font-bold text-gray-800">
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="relative group h-72 rounded-[2.5rem] overflow-hidden border-2 border-dashed border-gray-200 bg-gray-50 flex items-center justify-center transition-all hover:bg-gray-100 hover:border-gray-300">
                                ...
                                <template x-if="!mainPreview && existingMain">
                                    <div class="relative w-full h-full">
                                        <img :src="existingMain" class="w-full h-full object-cover cursor-zoom-in opacity-80" @click="showFull(existingMain)">
                                        <div class="absolute inset-0 flex items-center justify-center pointer-events-none">
                                            <span class="bg-black/50 text-white text-xs font-bold px-3 py-1 rounded-full backdrop-blur-sm">Current Cover</span>
                                        </div>
                                    </div>
                                </template>

                                <div x-show="!mainPreview && !existingMain" class="text-center">
                                    ...
                                    <p class="text-[10px] font-black text-gray-400 tracking-widest uppercase">Upload New Cover</p>
                                </div>
                                <input type="file" name="image" x-ref="mainInput" accept="image/jpeg,image/png,image/jpg,image/webp" class="absolute inset-0 opacity-0 cursor-pointer z-20" @change="updateMain" title="Click to replace cover">
                            </div>
                        </div>

                        {{-- Gallery --}}
                        <div class="space-y-4">
                            <label class="block text-sm font-black text-gray-700 uppercase">Gallery Photos</label>
                            <div class="grid grid-cols-2 gap-4 h-72 overflow-y-auto custom-scrollbar pr-2">

                                {{-- Existing Images --}}
                                <template x-for="(img, index) in existingGallery" :key="'ex-'+img.id">
                                    <div class="relative h-32 rounded-3xl overflow-hidden group shadow-sm border border-gray-100" x-show="!img.deleted">
                                        <img :src="img.url" class="w-full h-full object-cover cursor-zoom-in" @click="showFull(img.url)">
                                        <button type="button" @click="markDeleted(index)" class="absolute top-2 right-2 bg-black/50 backdrop-blur-md text-white p-1.5 rounded-full hover:bg-red-500 transition-colors z-10">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                                        </button>
                                        <input type="hidden" name="delete_images[]" :value="img.id" :disabled="!img.deleted">
                                    </div>
                                </template>

                                {{-- New Previews --}}
                                <template x-for="(item, index) in galleryPreviews" :key="'new-'+index">
                                    <div class="relative h-32 rounded-3xl overflow-hidden group shadow-sm border border-gray-100 border-green-200">
                                        <img :src="item.src" class="w-full h-full object-cover cursor-zoom-in" @click="showFull(item.src)">
                                        <button type="button" @click="removeGallery(index)" class="absolute top-2 right-2 bg-black/50 backdrop-blur-md text-white p-1.5 rounded-full hover:bg-red-500 transition-colors z-10">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M6 18L18 6M6 6l12 12"></path></svg>
                                        </button>
                                        <div class="absolute bottom-1 left-1 bg-green-500 text-white text-[8px] font-bold px-2 py-0.5 rounded-full">NEW</div>
                                    </div>
                                </template>
                ...
                <div class="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 p-8 sm:p-10">
                    ...
                    <div class="space-y-6">
                        <div class="relative group">
                            ...
                            <input type="text" name="location" id="location" value="{{ old('location', $farm->location) }}" required
                        ...
                        <input type="hidden" name="latitude" id="latitude" value="{{ old('latitude', $farm->latitude) }}">
                        <input type="hidden" name="longitude" id="longitude" value="{{ old('longitude', $farm->longitude) }}">
                    </div>
                </div>
                ...
                <div class="flex flex-col md:flex-row items-center justify-between bg-gray-900 rounded-[3rem] p-10 text-white gap-8 shadow-2xl">
                    <div class="text-center md:text-left">
                        <p class="font-black text-2xl leading-tight tracking-tighter uppercase">Update Listing</p>
                        <p class="text-gray-400 text-sm mt-1">Review your changes before saving.</p>
                    </div>
    ...
    <script>
        function farmGallery() {
            return {
                existingMain: @json($farm->main_image ? asset('storage/' . $farm->main_image) : null),
                mainPreview: null,

                existingGallery: @json($farm->images->map(function($img) {
                    return ['id' => $img->id, 'url' => asset('storage/' . $img->image_url), 'deleted' => false];
                })),
                ...
                markDeleted(index) {
                    this.existingGallery[index].deleted = true;
                },
                showFull(src) {
        ...
        document.addEventListener('DOMContentLoaded', function () {
            setTimeout(() => {
                let initLat = {{ $farm->latitude ?? 31.9454 }};
                let initLng = {{ $farm->longitude ?? 35.9284 }};

                const map = L.map('map', { scrollWheelZoom: false }).setView([initLat, initLng], {{ $farm->latitude ? 14 : 10 }});
                L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png').addTo(map);

                let marker;
                if ({{ $farm->latitude ? 'true' : 'false' }}) {
